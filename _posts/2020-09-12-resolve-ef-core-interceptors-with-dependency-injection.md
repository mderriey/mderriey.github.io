...
As mentioned [on Twitter by Joonas Westlin](https://twitter.com/JoonasWestlin/status/1314463453713707010), the `DefaultAzureCredential` class doesn't handle token caching, which means that your app could end up requesting a new token for each SQL connection.
